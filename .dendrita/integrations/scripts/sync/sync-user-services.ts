  ...
  try {
    ...
    if (!supabase.isConfigured()) {
      ...
      dendritaLogger.log({
        ...
        event_type: 'execute',
        event_description: 'Supabase not configured',
        status: 'error',
    ...
    dendritaLogger.log({
      ...
      component_path: scriptPath,
      event_type: 'execute',
      event_description: 'User services sync completed successfully',
      status: 'success',
      duration: Date.now() - startTime,
      triggered_by: scriptId,
      metadata: {
        total_users_synced: users.length,
        services_detected: {
          google_workspace: services.google_workspace,
    ...
    dendritaLogger.log({
      ...
      component_path: scriptPath,
      event_type: 'execute',
      event_description: 'User services sync failed',
      status: 'error',
      duration: Date.now() - startTime,
      error: error.message,
      triggered_by: scriptId,
