...
export async function findDriveFolder(config: SearchConfig): Promise<FolderReference[]> {
  try {
    ...
    if (!credentials.hasGoogleWorkspace()) {
      logger.error('Google Workspace no está configurado. Ver: .dendrita/integrations/SETUP.md');
      return [];
    }
...
- Ver: \`.dendrita/integrations/README.md\` para más información sobre el servicio de Google Drive
