...
This hook documents the expected behavior that Cursor must apply when the user requests to "dendritify" something (convert it into a dendrita system component).
...
**"Dendritify" means:** To convert something into a dendrita system component, following dendrita patterns, structure, and conventions.
...
Cursor must activate dendritify behavior when:

- ✅ User explicitly requests "dendritify" or "dendritificar" something
- ✅ User mentions "convertir en hook/agent/skill" or "convert to hook/agent/skill"
- ✅ User wants to make something "parte del sistema dendrita" or "part of dendrita system"
- ✅ User requests to "estructurar como dendrita" or "structure as dendrita"
- ✅ Working with temporary files that should become permanent dendrita components

**Activation condition:**

```markdown
IF (user mentions "dendritify" OR "dendritificar") OR 
   (user mentions "convertir en hook/agent/skill" OR "convert to hook/agent/skill") OR
   (user wants to make something "parte del sistema dendrita" OR "part of dendrita system") OR
   (user requests "estructurar como dendrita" OR "structure as dendrita")
THEN activate dendritify behavior
```

### 2. Dendritificar Process

When dendritify behavior is activated, Cursor must:

#### Step 1: Identify What to Dendritificar

1. **Identify the target:**
   - What is the user trying to dendritify?
...
2. **Review dendrita patterns:**
   ...
   - Review naming conventions from `.dendrita/users/[user-id]/config-estilo.json`
...
4. **Check naming conventions:**
   - Verify name follows `.dendrita/users/[user-id]/config-estilo.json` conventions
...
**CRITICAL:** Before completing dendritify, Cursor must verify that the component does not expose user or workspace data:
...
2. **Fix security issues:**
   - If issues are found, fix them before completing dendritify
...
2. **Update working context:**
   ...
   - Document what was dendritified
...
Before completing dendritify, verify:
...
If it's unclear what to dendritify:

1. **Ask user for clarification:**
   - What exactly should be dendritified?
...
1. **Review patterns:**
   - Read `.dendrita/users/[user-id]/config-estilo.json`
...
1. **Always review patterns first:**
   - Read `.dendrita/users/[user-id]/config-estilo.json` for naming
...
- `.dendrita/docs/TECHNICAL-PARADIGMS.md` - Security principles and data exclusion rules

---

**For Cursor:** This hook is a behavior reference. You must read this file and apply the documented logic when the user requests to "dendritify" something. Follow the systematic process to convert components into proper dendrita system components.
