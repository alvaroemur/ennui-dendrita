---
name: security
description: "Seguridad en Integraciones de Dendrita"
type: documentation
created:
...
#### Hook pre-push (verificación completa)

El repositorio incluye un hook `pre-push` más completo que:

1. **Verifica credenciales en commits**: Busca patrones de credenciales (API keys, tokens, secrets) en los commits que se van a pushear
2. **Ejecuta verificación de seguridad de dendrita**: Usa el script `check-dendrita-security.ts` para verificar datos sensibles
3. **Detecta archivos prohibidos**: Verifica que archivos sensibles (`.env.local`, `*.key`, etc.) no estén en los commits

El hook está ubicado en `.git/hooks/pre-push` y se ejecuta automáticamente antes de cada `git push`.

**Características:**
- ✅ Verifica solo los commits nuevos (no todo el historial)
- ✅ Detecta credenciales en diffs de commits
- ✅ Usa el script de verificación de seguridad de dendrita
- ✅ Bloquea el push si encuentra problemas
- ✅ Muestra reporte claro de problemas encontrados

**Para saltar la verificación (NO recomendado):**
```bash
git push --no-verify
```

**Nota:** El hook se crea automáticamente al configurar el repositorio. Si necesitas recrearlo, está documentado en `.git/hooks/pre-push`.
