...
if command -v ts-node &> /dev/null; then
    ...
    if command -v tsc &> /dev/null; then
        ...
        tsc .dendrita/archived/scripts/identify-test-debug-files.ts --outDir .dendrita/archived/scripts/dist --module commonjs --target es2020 --moduleResolution node --esModuleInterop --skipLibCheck
        
        if [ -f ".dendrita/archived/scripts/dist/identify-test-debug-files.js" ]; then
            node .dendrita/archived/scripts/dist/identify-test-debug-files.js
            exit 0
        fi
    fi
...
$RUNNER .dendrita/archived/scripts/identify-test-debug-files.ts

echo -e "\n${GREEN}✅ Identificación completada${NC}"
echo -e "   Revisa el reporte en: .dendrita/archived/test-debug-report.md"
